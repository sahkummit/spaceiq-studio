    ...
    <div class="container mx-auto px-6 xl:px-12" style="max-width:1536px">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-5" style="min-height:680px;">
            ...
            <a href="{{ route('service.show', $service->slug) }}"
               ...
               style="min-height:540px;">
                ...
                <!-- BG image -->
                <div class="absolute inset-0 bg-cover bg-center transition-transform duration-[1200ms] ease-out group-hover:scale-110 {{ $is360Card ? 'group-hover:blur-sm' : '' }}"
                ...
                <!-- Number tag -->
                <span class="absolute top-6 left-6 text-[10px] font-black tracking-[0.3em] text-accent-400/60 uppercase">{{ $num }}</span>

                <!-- Bottom content -->
                <div class="absolute bottom-0 left-0 right-0 p-8">
                    <p class="text-accent-400 text-[10px] font-black tracking-[0.3em] uppercase mb-2">Featured</p>
                    <h3 class="text-white font-display font-black text-3xl uppercase tracking-tight mb-3 leading-tight">{{ $service->title }}</h3>
            ...
            @else
            @if($index === 1)
            {{-- Right 4-card panel wrapper (opens on index 1, closes after index 4) --}}
            ...
            <a href="{{ route('service.show', $service->slug) }}"
               ...
               style="aspect-ratio:4/3;">
                ...
                <div class="absolute inset-0 bg-cover bg-center transition-transform duration-700 ease-out group-hover:scale-110 {{ $is360Card ? 'group-hover:blur-sm' : '' }}"
                     style="background-image:url('{{ $cardImageUrl }}');"></div>
                ...
                <span class="absolute top-4 left-4 text-[9px] font-black tracking-[0.3em] text-accent-400/50 uppercase">{{ $num }}</span>

                <div class="absolute bottom-0 left-0 right-0 p-4">
                    <h3 class="text-white font-display font-bold text-sm uppercase tracking-wide leading-tight mb-1">{{ $service->title }}</h3>
            ...
            @if($index === $homeServices->count() - 1 || $index === 4)
            </div>{{-- close right panel grid --}}
            @endif
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/5 rounded-2xl overflow-hidden mt-5">
            ...
            <div class="bg-brand-900/60 px-6 py-5 text-center hover:bg-brand-800/60 transition-colors duration-300">
                <p class="text-2xl font-display font-black text-white mb-1">{{ $stat['value'] }}</p>
        ...
        <div class="text-center mt-10">
            <a href="{{ route('service.show', 'exterior-renders') }}" class="inline-flex items-center gap-2 text-accent-400 hover:text-white text-sm font-semibold uppercase tracking-widest border-b border-accent-400/40 hover:border-white pb-1 transition-all duration-300">
                View All Services
