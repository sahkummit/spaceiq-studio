                                ...
                                <div class="w-full md:w-1/2 relative overflow-hidden h-[400px] bg-brand-950">
                                    @if($panoramaMedia)
                                        <div id="home-panorama-{{ $panoramaMedia->id }}" 
                                             ...
                                             x-data
                                             x-init="$nextTick(() => {
                                                 pannellum.viewer('home-panorama-{{ $panoramaMedia->id }}', {
                                                     type: 'equirectangular',
                                                     panorama: '{{ parse_url(Storage::url($panoramaMedia->file_path), PHP_URL_PATH) }}',
                                                     autoLoad: true,
                                                     compass: false,
                                                     autoRotate: -2,
                                                     mouseZoom: false,
...
<section id="contact" class="py-24 relative bg-brand-950 overflow-hidden reveal">
    <div class="absolute inset-0 z-0 bg-[url('/img/interior_render.png')] bg-cover bg-center opacity-20"></div>
    <div class="container mx-auto px-6 max-w-4xl text-center relative z-10 bg-brand-900/80 backdrop-blur-md p-16 rounded-sm border border-white/10 shadow-2xl">
        <h2 class="text-4xl md:text-5xl font-display font-bold mb-6 text-white uppercase tracking-wider">Ready to make your vision a reality?</h2>
        
        <a href="{{ route('contact') }}" class="btn-glow inline-block px-12 py-5 bg-accent-500 hover:bg-accent-400 text-brand-950 font-bold uppercase tracking-widest text-sm transition-colors duration-300 mt-6 shadow-xl">
            Book a Consultation
        </a>
    </div>
</section>
